...
func (c *Challenge) Solve(authz acme.Authorization) error {
	...
	log.Printf("[%s] acme: Trying to solve TLS-ALPN-01", challenge.GetTargetedDomain(authz))
	...
	defer func() {
		...
		if err != nil {
			log.Printf("[%s] acme: cleaning up failed: %v", challenge.GetTargetedDomain(authz), err)
		}
	}()
