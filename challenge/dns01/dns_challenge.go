...
func NewChallenge(core *api.Core, validate ValidateFunc, provider challenge.Provider, opts ...ChallengeOption) *Challenge {
	...
	for _, opt := range opts {
		err := opt(chlg)
		if err != nil {
			log.Printf("challenge option error: %v", err)
		}
	}
...
func (c *Challenge) PreSolve(authz acme.Authorization) error {
	...
	log.Printf("[%s] acme: Preparing to solve DNS-01", domain)
...
func (c *Challenge) Solve(authz acme.Authorization) error {
	domain := challenge.GetTargetedDomain(authz)
	log.Printf("[%s] acme: Trying to solve DNS-01", domain)
	...
	log.Printf("[%s] acme: Checking DNS record propagation using %+v", domain, recursiveNameservers)
	...
	err = wait.For("propagation", timeout, interval, func() (bool, error) {
		...
		if !stop || errP != nil {
			log.Printf("[%s] acme: Waiting for DNS record propagation.", domain)
		}
		return stop, errP
	})
	if err != nil {
...
func (c *Challenge) CleanUp(authz acme.Authorization) error {
	log.Printf("[%s] acme: Cleaning DNS-01 challenge", challenge.GetTargetedDomain(authz))
