...
func (s *ProviderServer) serve(domain, token, keyAuth string) {
	...
	mux.HandleFunc(path, func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodGet && s.matcher.matches(r, domain) {
			...
			log.Printf("[%s] Served key authentication", domain)
		} else {
			log.Printf("Received request for domain %s with method %s but the domain did not match any challenge. Please ensure your are passing the %s header properly.", r.Host, r.Method, s.matcher.name())
