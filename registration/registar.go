...
// WARNING: will be remove in the future (acme.ExtendedAccount), https://github.com/go-acme/le/issues/855.
...
func (r *Registrar) Register(options RegisterOptions) (*Resource, error) {
	...
	if r.user.GetEmail() != "" {
		log.Printf("acme: Registering account for %s", r.user.GetEmail())
...
func (r *Registrar) QueryRegistration() (*Resource, error) {
	...
	log.Printf("acme: Querying account for %s", r.user.GetRegistration().URI)
...
func (r *Registrar) DeleteRegistration() error {
	...
	log.Printf("acme: Deleting account for %s", r.user.GetEmail())
...
func (r *Registrar) ResolveAccountByKey() (*Resource, error) {
	log.Printf("acme: Trying to resolve account by key")
