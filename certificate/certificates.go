...
func (c *Certifier) Obtain(request ObtainRequest) (*Resource, error) {
	...
	if request.Bundle {
		log.Printf("[%s] acme: Obtaining bundled SAN certificate", strings.Join(domains, ", "))
	} else {
		log.Printf("[%s] acme: Obtaining SAN certificate", strings.Join(domains, ", "))
	}
	...
	log.Printf("[%s] acme: Validations succeeded; requesting certificates", strings.Join(domains, ", "))
...
func (c *Certifier) ObtainForCSR(request ObtainForCSRRequest) (*Resource, error) {
	...
	if request.Bundle {
		log.Printf("[%s] acme: Obtaining bundled SAN certificate given a CSR", strings.Join(domains, ", "))
	} else {
		log.Printf("[%s] acme: Obtaining SAN certificate given a CSR", strings.Join(domains, ", "))
	}
...
func (c *Certifier) checkResponse(order acme.ExtendedOrder, certRes *Resource, bundle bool, preferredChain string) (bool, error) {
	...
	if preferredChain == "" {
		log.Printf("[%s] Server responded with a certificate.", certRes.Domain)
	...
	for link, cert := range certs {
		...
		if ok {
			log.Printf("[%s] Server responded with a certificate for the preferred certificate chains %q.", certRes.Domain, preferredChain)
	...
	log.Printf("lego has been configured to prefer certificate chains with issuer %q, but no chain from the CA matched this issuer. Using the default certificate chain instead.", preferredChain)
...
func (c *Certifier) Renew(certRes Resource, bundle, mustStaple bool, preferredChain string) (*Resource, error) {
	...
	log.Printf("[%s] acme: Trying renewal with %d hours remaining", certRes.Domain, int(timeLeft.Hours()))
...
func sanitizeDomain(domains []string) []string {
	...
	for _, domain := range domains {
		...
		if err != nil {
			log.Printf("skip domain %q: unable to sanitize (punnycode): %v", domain, err)
		} else {
