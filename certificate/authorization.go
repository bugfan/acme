...
func (c *Certifier) getAuthorizations(order acme.ExtendedOrder) ([]acme.Authorization, error) {
	...
	for i, auth := range order.Authorizations {
		log.Printf("[%s] AuthURL: %s", order.Identifiers[i].Value, auth)
	}
...
func (c *Certifier) deactivateAuthorizations(order acme.ExtendedOrder) {
	for _, authzURL := range order.Authorizations {
		...
		if err != nil {
			log.Printf("Unable to get the authorization for: %s", authzURL)
			continue
		}

		if auth.Status == acme.StatusValid {
			log.Printf("Skipping deactivating of valid auth: %s", authzURL)
			continue
		}

		log.Printf("Deactivating auth: %s", authzURL)
		if c.core.Authorizations.Deactivate(authzURL) != nil {
			log.Printf("Unable to deactivate the authorization: %s", authzURL)
		}
	}
}
